            ...
            <div class="col-md-12">
              ...
              <table class="table">
                  ...
                  @forelse( $orders as $order)
                      <tr>
                          <td><a href="{{ route('customer-orders.edit', $order->id) }}">{{ $order->padded_id }}</a></td>
                          <td>{{ $order->created_at}}</td>
                          <td>{{ $order->human_status}}</td>
                          <td>
                              Total : <strong>{{number_format($order->total_payment)}}</strong><br>
                              Transfer ke : {{ config('bank-accounts')[$order->bank]['bank']}} An. {{ config('bank-accounts')[$order->bank]['name'] }} {{ config('bank-accounts')[$order->bank]['number'] }}<br>
                              Dari : {{$order->sender}}
                          </td>
                          <td>
